...
> 记录用 Go 开发 CLI 工具过程中遇到的知识点，以 `openTaurus` 项目为主线。
...
这样设计的好处是：添加新命令（比如 `openTaurus backup`）只需在 `cmd/` 下新增一个文件，`main.go` 完全不用动。
...
- `rootCmd` 是根命令（`Use: "openTaurus"`）
- `AddCommand` 把子命令注册进去
- 没有这一行，`openTaurus configure` 永远找不到目标
...
在终端输入 `./openTaurus configure` 时，`Execute()` 像交警一样：
