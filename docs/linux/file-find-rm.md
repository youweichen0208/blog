...
| 选项 | 全称 | 作用 |
|------|------|------|
| `-l` | long | 详细列表（权限、所有者、大小、修改时间） |
| `-a` | all | 显示所有文件，包括隐藏文件（`.` 开头的，如 `.env`、`.git`） |
...
```
-rw-r--r--  1 www-data www-data  1234 Jun 14 10:30 nginx.conf
drwxr-xr-x  3 root     root      4096 Jun 14 09:00 data
```
...
`ll` 是 `ls -alF`（或 `ls -l`）的快捷别名（alias），不是原生命令。可以通过 `alias ll` 查看具体映射。
