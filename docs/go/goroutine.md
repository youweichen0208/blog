...
**执行流程拆解：**
以其中一个Worker为例，它的生命周期是这样的：

1. **启动**：go func被调用。
2. **注册**：执行到`defer wg.Done()`，Go把这个任务压入到协程的“延迟栈”中。
3. **工作**：执行`fmt.Printf("Worker %d 开始\n", id)`和time.Sleep。
4. **打印结束**：执行`fmt.Printf("Worker %d 完成\n", id)`。
5. **触发defer**: 匿名函数代码全部跑完，准备退出。此时开始弹出并执行defer任务，即调用`wg.Done()`
6. **真正的结束**：该协程销毁。

**为什么一定要用defer？**
在这段并发代码中，defer的作用是**兜底**。

- 保证计数器一定减少：如果你的Worker代码中间发生了panic，或者函数中途有多个return分支，defer都能确保`wg.Done()`被调用。
- 防止死锁：如果没有执行`wg.Done()`, main函数里的`wg.Wait()`就会永远阻塞在那，导致程序死锁(Deadlock)。
...
1. 时间线的赛跑
   当写下`go func() { ... }()`时，Go调度器会把这个任务放入队列。主线程（main）和新开的协程(goroutine)是同时跑的，谁先谁后并不确定。

   看看这个错误的时间线：
   1. **主线程** 看到 `go func()`，把它扔给调度器（还没开始跑）。
   2. **主线程**：紧接着执行下一个`wg.Wait()`
   3. 主线程：发现此时`WaitGroup`的计数器竟然是**0**（因为协程还没开始跑，里面的`wg.Add(1)`还没执行）。
   4. **主线程**：既然计数器时0，`Wait()`认为任务全做完了，直接放行，结束程序！
   5. 子协程（绝望的）：正准备执行`wg.Add(1)`，结果主程序已经关门熄灯了。
