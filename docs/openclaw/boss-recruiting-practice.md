...
#### 坑 3：`INTAKE_ONLY=true` 时不要再被 `java` 之类关键词分流

真实跑下来，一个高频误判是：

- 候选人明明是主动投递
- 学校也达标
- 但消息里包含 `Java`、`后端`、`Python` 之类关键词
- 结果脚本先把它归进 `tech_stack_question`

这样会导致它根本没进入 school gate，后面既不会自动发采集模板，也不会落到“待回复”名单里。

后来稳定下来的做法是：

1. 在 `INTAKE_ONLY=true` 时，未读学生消息优先且只走院校筛选
2. 不再把 `java`、`go`、`python`、`后端` 这些自我介绍常见词当成技术栈提问的触发词

也就是说，在 student intake 模式下，路由优先级应该是：
...
自动采集解决的是“学校通过后自动发模板”，但真实使用里还有另一个问题：

- 有些候选人学校达标
- 页面上只是 `已读`，并没有真正回复
- 如果只盯未读红点，很容易漏掉

所以后来又单独加了一个“最近 20 个院校达标会话是否已回复”的巡检脚本：

```bash
npm run audit-recent-school-pass-replies -- --limit 20
```

它做的不是“看有没有未读”，而是：

1. 打开最近 20 个聊天
2. 对每个聊天重新跑 school gate
3. 只保留院校达标的会话
4. 保守判断是否已经明确回复

这里专门强调“保守判断”，因为一开始踩过一个坑：
...
- 只有明确识别出“你发出的消息 DOM”，才算 `replied`
- 否则宁可记成 `pending_reply`
...
后来补上的做法是直接把常见中文名写进 alias：

```json
"加州大学伯克利分校": "University of California, Berkeley"
```

否则像“点评了”这类会话，看起来像是美国 Top 100 学校，实际上脚本会直接漏掉。
